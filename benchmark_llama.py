...
def run_benchmark(data):
    print(f"\n=============================================")
    print(f"=== Starting benchmark for LLAMA ===")
    print(f"=============================================")
    
    base_model = "aisingapore/Llama-SEA-LION-v3.5-8B-R"
    adapter_path = "./output/llama-8b/final"
    db_path = "benchmark_llama.db"
    excel_path = "benchmark_llama.xlsx"
    
    # Try to load model
    try:
        model, tokenizer = llama_inf.load_model(adapter_path, base_model)
    ...
    for i, item in enumerate(data):
        ...
        try:
            model_response = llama_inf.generate(model, tokenizer, question)
        except Exception as e:
    ...
    print(f"=== Finished benchmark for Llama ===")
...
def main():
    parser = argparse.ArgumentParser(description="Run benchmark for Llama")
