...
def run_benchmark(data):
    print(f"\n=============================================")
    print(f"=== Starting benchmark for GEMMA ===")
    print(f"=============================================")
    
    base_model = "aisingapore/Gemma-SEA-LION-v4-4B-VL"
    adapter_path = "./output/gemma-4b/final"
    db_path = "benchmark_gemma.db"
    excel_path = "benchmark_gemma.xlsx"
    
    # Try to load model
    try:
        model, tokenizer = gemma_inf.load_model(adapter_path, base_model)
    ...
    for i, item in enumerate(data):
        ...
        try:
            model_response = gemma_inf.generate(model, tokenizer, question)
        except Exception as e:
    ...
    print(f"=== Finished benchmark for Gemma ===")
...
def main():
    parser = argparse.ArgumentParser(description="Run benchmark for Gemma")
