#!/usr/bin/env python3
"""
Full SFT (no QLoRA) for Llama-SEA-LION-v3.5-8B-R.

All parameters are trained in bf16. Requires ~60GB+ VRAM.
Use DeepSpeed ZeRO-3 or FSDP for multi-GPU if needed.

Usage:
  python llama/llama_full_sft.py
  python llama/llama_full_sft.py --epochs 2 --lr 2e-5
  accelerate launch llama/llama_full_sft.py
"""
...
OUTPUT_DIR = "./output/llama-8b-full-sft"
DATA_DIR = "./data"
MAX_SEQ_LENGTH = 512
EPOCHS = 1
BATCH_SIZE = 1
...
def train(args):
    ...
    sft_config = SFTConfig(
        ...
        max_grad_norm=1.0,
        report_to="wandb",
        run_name="llama-sealion-8b-full-sft",
        max_length=max_seq,
        packing=True,
    )
