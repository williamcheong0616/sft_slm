#!/usr/bin/env python3
"""
Full SFT (no QLoRA) for Qwen-SEA-LION-v4-8B-VL.

All parameters are trained in bf16. Requires ~60GB+ VRAM.
Use DeepSpeed ZeRO-3 or FSDP for multi-GPU if needed.

Usage:
  python qwen/qwen_full_sft.py
  python qwen/qwen_full_sft.py --epochs 2 --lr 2e-5
  accelerate launch qwen/qwen_full_sft.py
"""
...
OUTPUT_DIR = "./output/qwen-8b-full-sft"
DATA_DIR = "./data"
MAX_SEQ_LENGTH = 512
EPOCHS = 1
BATCH_SIZE = 1
...
def make_formatting_func(tokenizer):
    """Format examples using Qwen's chat template."""
...
def train(args):
    ...
    sft_config = SFTConfig(
        ...
        max_grad_norm=1.0,
        report_to="wandb",
        run_name="qwen-sealion-8b-full-sft",
        max_length=max_seq,
        packing=True,
    )
